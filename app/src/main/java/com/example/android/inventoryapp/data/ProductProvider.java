...
public class ProductProvider extends ContentProvider {
    ...
    //Update items(products) in the database with the given content values.
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        //If the {@link ProductEntry#COLUMN_PRODUCT_NAME} key is present , check that the name is not null
        ...
        //If the {@link ProductEntry#COLUMN_PRODUCT_QUANTITY} key is present , check that is positive
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_QUANTITY)) {
            Integer quantity = values.getAsInteger(ProductEntry.COLUMN_PRODUCT_QUANTITY);
            if (quantity == null && quantity < 0) {
                throw new IllegalArgumentException("Quantity needs to be a positive number");
            }
        }

        //If the {@link ProductEntry#COLUMN_PRODUCT_PRICE} key is present , check that is positive
